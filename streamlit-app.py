...
with Maq1:
  ...
  with row0_col1:
    ...
    with row1_col2:
      ...
      st.write(fig)
  with row0_col2:
    fig1 = go.Figure(go.Indicator(mode = "gauge+number+delta",value = last_ratio_daily, domain = {'x': [0, 1], 'y': [0, 1]}, delta = {'reference': 40}, title = {'text': "Daily"}, gauge = {'axis': {'range': [0, 100]}}))
    fig1.update_layout(width=500,height=200,margin=dict(l=20, r=20, b=20, t=50))
    #st.write(fig)
    fig2 = go.Figure(go.Indicator(mode = "gauge+number+delta",value = last_ratio_weekly,domain = {'x': [0, 1], 'y': [0, 1]},delta = {'reference': 45},title = {'text': "Weekly"}, gauge = {'axis': {'range': [0, 100]}}))
    fig2.update_layout(width=500,height=200,margin=dict(l=20, r=20, b=20, t=50))
    #st.write(fig)
    fig3 = go.Figure(go.Indicator(mode = "gauge+number+delta",value = last_ratio_monthly,domain = {'x': [0, 1], 'y': [0, 1]},delta = {'reference': 35},title = {'text': "Monthly"}, gauge = {'axis': {'range': [0, 100]}}))
